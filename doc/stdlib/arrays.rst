...
.. function:: @sub A[inds...]
   ...
   Creates a ``SubArray`` from an indexing expression. This can only be applied directly to a reference expression (e.g. ``@sub A[1,2:end]``\ ), and should *not* be used as the target of an assignment (e.g. ``@sub(A[1,2:end]) = ...``\ ).
