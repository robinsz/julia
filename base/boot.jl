...
unsafe_write(io::IO, x::Ptr{UInt8}, nb::UInt) =
    ccall(:jl_uv_puts, Void, (Ptr{Void}, Ptr{UInt8}, UInt), io_pointer(io), x, nb)
unsafe_write(io::IO, x::Ptr{UInt8}, nb::Int) =
    ccall(:jl_uv_puts, Void, (Ptr{Void}, Ptr{UInt8}, Int), io_pointer(io), x, nb)
write(io::IO, x::UInt8) =
    ccall(:jl_uv_putb, Void, (Ptr{Void}, UInt8), io_pointer(io), x)
write(io::IO, x::String) =
    unsafe_write(io, ccall(:jl_array_ptr, Ptr{UInt8}, (Any,), x.data), sizeof(x.data))
...
print(io::IO, a::ANY...) = for x in a; print(io, x); end
